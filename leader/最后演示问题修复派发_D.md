...
5. 点击知识图谱节点后，右侧详情/跳转内容有时会变成其他教材的内容，而不是当前节点所属的原教材内容。
...
## P0-3：节点点击详情必须绑定当前教材和当前节点

用户反馈：点击知识图谱节点之后，右侧内容或跳转内容有时会显示其他教材里的内容，而不是原节点内容。

重点检查：

- `frontend/src/App.jsx`
- `frontend/src/components/GraphView.jsx`
- 必要时 `frontend/src/api.js`

修复要求：

1. `GraphView` 点击事件传给 `onNodeClick` 的必须是当前过滤后图谱里的完整 node 对象。
2. 切换左侧教材时，必须清空旧的 `selectedNode`，避免继续显示上一教材节点详情。
3. `handleNodeClick` 里要保留并展示当前 node 的真实字段，不要根据 name 去全局列表里二次查找，否则同名概念会跳到其他教材。
4. 节点详情必须显示：
   - 节点名称
   - 定义/描述
   - 当前教材来源
   - 章节
   - 页码
   - category/frequency/degree
5. 如果同名概念存在于多本教材，点击当前教材图谱里的节点时，只显示当前教材那条节点数据。
6. 如果节点有 `sources` 数组，只能展示与当前 `selectedTextbook` 匹配的 source；不要把其他教材 source 混进详情。
7. 如果当前节点在过滤后图谱里被移除，详情区域要自动清空。
...
```jsx
const handleSelectTextbook = (tb) => {
  setSelectedTextbook(tb);
  setSelectedNode(null);
};
```

- 左侧教材点击使用 `handleSelectTextbook(tb)`，不要直接 `setSelectedTextbook(tb)`。
- `GraphView` 在 `selectedTextbook` 变化后重新过滤图谱，并在必要时调用 `onNodeClick(null)` 或由 App 清空详情。
- `handleNodeClick` 不要用 node.name 查全局节点，只使用事件传入的 `nodeData`。

验收：

- 选中 A 教材，点击图谱节点，右侧详情来源是 A。
- 切换到 B 教材后，旧详情消失。
- 点击 B 教材节点，右侧详情来源是 B。
- 同名概念不会跳到其他教材内容。

## P0-4：RAG 引用来源不能显示 object
...
2. 图谱按教材过滤是否通过。
3. 噪声节点过滤规则有哪些。
4. 节点点击详情是否已绑定当前教材，是否清除了跨教材跳转问题。
5. RAG 引用是否已从 object 改为可读来源。
6. 报告生成后是否自动显示正文。
7. build 是否通过。
8. 剩余风险，不超过 5 条。
